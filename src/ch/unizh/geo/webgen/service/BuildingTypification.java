...
import com.vividsolutions.jump.feature.FeatureCollection;
	...
	public class BuildingTypification extends AWebGenAlgorithm implements IWebGenAlgorithm {
		...
		public void run(WebGenRequest wgreq) {
			...
			if(haveSameGeomTypes(fc)) {
				if (maxnumber > 1) {
		    		...
		    		if(feats.size() == 2) createFeatureStv((Feature)feats.get(0),(Feature)feats.get(1));
		    		wgreq.addResult("result", fcnew);
				}
			}
		}
...
//	    		System.out.println("Number of objects:\t" + fcnew.size()+"; actual minimum length:\t "+actualLength);    		
		...
		public void initTriangulation(FeatureCollection fc) {
	        ...
	        if(WEIGHTED_DELAUNAY == true  || COMPARE == true) {
	        	...
	        	while(triit.hasNext()) {
	        		...
	        		if((te.node1.featureList.size() > 1) && (te.node1.featureList.size() > 1)) {
	        			Feature n1 = (Feature)te.node1.featureList.get(1);
	        			Feature n2 = (Feature)te.node2.featureList.get(1);
		...
		protected Feature calculateFeatureStv(TEdge shortestEdge) {
			//Stellverter 1 & 2
			Feature ft1 = (Feature)shortestEdge.node1.featureList.get(1);
			Feature ft2 = (Feature)shortestEdge.node2.featureList.get(1);
			
			Feature fnew = createFeatureStv(ft1, ft2);
		...
		protected Feature createFeatureStv(Feature ft1, Feature ft2) {
			Feature fnew = null;
			...
			fnew = (Feature)ftWork.clone();
			
			//Objekte an den Stellvertreterplatz verschieben
			Coordinate spTrans = (Coordinate)ftWork.getGeometry().getCentroid().getCoordinate().clone();
			spTrans.x -=  sp_x;
			spTrans.y -=  sp_y;
	    ...
	    public InterfaceDescription getInterfaceDescription() {
			InterfaceDescription id = new InterfaceDescription("BuildingTypification", "neun", "operator",
					"",
					"BuildingTypification",
					"typify buildings or other features",
					"1.0",
					new String[] {"ica.genops.cartogen.Typification"});
			id.visible = true;
			...
			id.addOutputParameter("result", "FeatureCollection", new AttributeDescription("GEOMETRY", "GEOMETRY", allowed), "typified geometries");
			return id;
		}
	}