...
public class Eval_GMFeat_All_Constrained extends AWebGenAlgorithm implements IWebGenAlgorithm {
	...
	private void callServices(WebGenRequest wgreq) {
		Double[] partitionCost = new Double[9];
		...
		(new Eval_GMFeat_MinDistConstrained()).run(wgreq);
		...
		(new Eval_GMFeat_Diff_BlackWhiteRatio()).run(wgreq);
		partitionCost[8] = wgreq.getResultDouble("severity");
		wgreq.addResult("severityBlackWhiteRatio", wgreq.getResult("severity"));
		if(wgreq.getResult("result") != null) wgreq.addParameter("geom", wgreq.getResult("result"));
	...
	public InterfaceDescription getInterfaceDescription() {
		InterfaceDescription id = new InterfaceDescription("Eval_GMFeat_All_Constrained", "neun", "support",
				"",
				"Eval_GMFeat_All_Constrained",
		...
		id.addInputParameter("roaddist", "DOUBLE", 10.0, 0.0, Double.POSITIVE_INFINITY, "minimum road distance");
